...
## B29 — `merge_branch` subsumption drops class types in favour of `mixed`

**Status:** open
**Severity:** medium (false-positive `unresolved_member_access`)

**Symptom:** After `assert($data instanceof stdClass)`, inserting
any `if` block (even `if (true) {}`) before a subsequent member
access causes `$data` to lose its type.  The diagnostic engine
reports `unresolved_member_access` on `$data->status`.
...
Without the `if (true) {}` block, `$data` resolves correctly.

**Root cause:** `ScopeState::merge_branch` in `forward_walk.rs`
removes entries whose type is a subset of another entry's type.
After the `if` block merges, `$data` has two entries: `stdClass`
(from the narrowed assert) and `mixed` (from the pre-narrowed
scope carried through the implicit else path).  Because
`stdClass ⊆ mixed`, the subsumption logic drops `stdClass` and
keeps only `mixed`.  The diagnostic engine cannot resolve member
accesses on `mixed`, producing a false positive.

**Constraint:** Simply preventing `mixed` from subsuming class
types fixes this case but breaks `instanceof` narrowing on
`mixed` variables (e.g. `if ($mixed instanceof Foo)` should
produce `Foo`, not `Foo|mixed`).  The fix needs to distinguish
between "mixed leaked from a branch merge" and "mixed is the
actual pre-narrowing type that should be replaced by instanceof".

**Affected projects:** Luxplus Shared (1 error), potentially any
project where `assert instanceof` is followed by an `if` block.
